...
├── doctor_finder.py          # Nearby doctor/clinic lookup via OpenStreetMap Overpass API
...
│   ├── login.html
│   ├── register.html
│   ├── chat.html                # Main chat UI + PDF upload + nearby doctors button
...
| **PDF RAG** | On upload, `rag.py` extracts text (`pypdf`), splits it into overlapping chunks, and stores chunks per user in SQL. On each chat message, it TF-IDF-vectorizes the question against that user's chunks and returns the most relevant snippet(s) — classic sparse-vector retrieval, no external LLM call required. |
| **Nearby doctors** | The browser's `navigator.geolocation` API gets your lat/long, which is sent to `/nearby-doctors`. `doctor_finder.py` queries the free OpenStreetMap **Overpass API** for `amenity=doctors/clinic/hospital/pharmacy` nodes nearby and sorts them by haversine distance. |
